...
public class SingTask04 {
	public static void main(String[] args) {
		...
		// 암은 금액으로 음료 구매 가능 여부 추가(한 곡당 금액 200원, 음료 금액은 100원)
		// 총 부른 곡 수에 따른 보너스 지급
		// 5곡을 부르면 1곡 추가 제공

		// 1) 입력클래스 import
		Scanner sc = new Scanner(System.in);
		// 2) 정수형 상수2개 선언, 정수형 6개 선언(금액입력, 노래곡수, 잔돈, 음료마실경우노래곡수, 음료마실경우잔돈, 보너스곡)
		...
		// 8) 음료를 위해 한 곡 줄이는 경우
		// if(노래곡수 > 0) && (금액 - ((노래곡수 - 1) * 노래 가격 >= 음료 금액))
		else if (song > 0 && (money - (drinkSong * PRICE)) >= PRICE_DRINK) {
			// 9) 음료 마실경우 노래곡수 = 노래곡수 - 1;
//			drinkSong = song - 1;
		...
		// 보너스곡 = 노래곡수 / 5;
		// 12) if(보너스곡 > 0){
		// 		보너스로 몇곡이 추가되어 총 몇곡을 부를 수 있었습니다.
		// }
		bonus = song / 5;
		if(bonus > 0) {
